...
'''
urltail = '&results=25&order=Sales&ownership=Both&direction=DESC&showtotalsales=1'
urltail += '&shownasales=1&showpalsales=1&showjapansales=1&showothersales=1&showpublisher=1'
urltail += '&showdeveloper=1&showreleasedate=1&showlastupdate=0&showvgchartzscore=0&showcriticscore=1&showuserscore=1&showshipped=1'

print(urlhead+urltail)
def find_attrs(a):
    if 'href' in a.attrs:
        
        if a.attrs['href'].startswith('https://www.vgchartz.com/game/'):
            
            return True
    return False
for page in range(101, pages):
    ...
    for tag in game_tags:
        ...
        release_year = data[14].string.split()[-1]
        ...
        while(tryagin):
            try:       
                ...
                print('faiur occur, attemt try again')
                tryagin += 0.5
                time.sleep(2+tryagin)
...
df = df[[
    ...
    'NA_Sales', 'PAL_Sales', 'JP_Sales', 'Other_Sales', 'Global_Sales','Total_Shipped']]
